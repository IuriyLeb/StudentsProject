...
def call_random_student_on_weight(students_dict,weight):
	...
	while user_input!="q":
		...
		if user_input=="No":
			...
			del weight[key]
			new_students_dict={ind:students_dict[i] for ind,i in enumerate(sorted(students_dict.keys()),1)}
			new_weight={ind:weight[i] for ind,i in enumerate(sorted(weight.keys()),1)}
			flag=0
			break
	if flag==0:	
		call_random_student_on_weight(new_students_dict,new_weight)
